 ...
 //  var tabs = ['Orders'];
    ...
    for (var j = 0; j < tabs.length; j++) {
        ...
        if (tabs[j] == 'Orders') {
            ...
            var machinesP = base.getData("Machines");
            if (!data) {
                continue;
            }
            Logger.log(data);
            ...
            var keys = ['row', 'menu', 'notes', 'move', 'orderdate', 'runtime', 'machineP', 'machineL', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'orderID', 
                        'productcode', 'productdescription', 'priority', 'customer','brand', 'bottles', 'btype', 'lid', 'tubeType', 'packagingType.name', 'usecomb', 'combs', 
                         'premixedSerum', 'premixedStimulant',  'backtubed', 'production_status', 'printing_status', 'labeling_status', 'packaging_status', 'final_status'
            ];

            var values = [];
            values.push(heads);
            values = values.concat(exportRows(keys, data, machinesP));
         ...
         var keys = [ 'orderdate', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'orderID', 'productcode', 'productdescription', 'priority', 'customer',
            'brand', 'bottles', 'btype', 'lid', 'tubeType', 'packagingType.name', 'usecomb', 'combs', 'starttime', 'CompletionDate', 'Completed', 'Location', 'machineP'
        ];

        var heads = ['Order Date', 'Batch', 'Serum Batches', 'Stimulant Batches', 'Order ID', 'Code', 'Description', 'Priority', 'Customer', 'Brand', 'Bottles', 'Bottle Type', 'Lid Type', 'Tube Type', 'Pack Type', 'Use Comb', 'Combs',
       ...
       var keys = [ 'production_status', 'orderdate', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'numLabelsBottles', 'numLabelsTubes', 'expDate', 'orderID', 'productcode', 'productdescription', 'priority', 'customer',
        'brand', 'bottles', 'btype', 'lid', 'tubeType', 'packagingType.name', 'botlabel', 'boxname.name', 'starttime', 'CompletionDate', 'Completed', 'Location'
    ];

    var heads = ['Production Status', 'Order Date', 'Batch', 'Serum Batches', 'Stimulant Batches', 'Bottle Labels QTY', 'Cylinder Labels QTY', 'Exp. Date', 'Order ID', 'Code', 'Description', 'Priority', 'Customer', 'Brand',
    ...
    var keys = [ 'production_status', 'orderdate', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'orderID', 'productcode', 'productdescription', 'priority', 'customer',
        'brand', 'bottles', 'btype', 'lid', 'tubeType', 'packagingType.name', 'botlabel', 'boxname.name', 'starttime', 'CompletionDate', 'Completed', 'Location'
    ];

    var heads = [ 'Production Status', 'Order Date', 'Batch', 'Serum Batches', 'Stimulant Batches', 'Order ID', 'Code', 'Description', 'Priority', 'Customer', 'Brand',
        'Bottles', 'Bottle Type', 'Lid Type', 'Tube Type', 'Pack Type', 'Label', 'Outer Box',
        'Start Date', 'Completion Date', 'Production Completed', 'Location'
    ];
            var values = [];
            ...
            if (!data) {
              ...
              var keys = [  'production_status', 'printing_status', 'orderdate', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'orderID', 'productcode', 'productdescription', 'PRINTCODE', 'priority', 'customer',
        'brand', 'bottles', 'btype', 'lid', 'tubeType', 'packagingType.name', 'boxname.name', 'starttime', 'CompletionDate', 'final_status', 'Location'
    ];

    var heads = ['Production Status', 'Printing Status', 'Order Date', 'Batch', 'Serum Batches', 'Stimulant Batches', 'Order ID', 'Code', 'Description', 'Packaging Code', 'Priority', 'Customer', 'Brand',
        ...
        'Start Date', 'Completion Date', 'Packaging Completed', 'Location'
          ...
          var keys = [ 'orderdate', 'batch', 'serumBatchCode', 'stimulantBatchCode', 'orderID', 'priority', 'productcode', 'productdescription', 'PRINTCODE', 'customer',
          ...
          var heads = ['Order Date', 'Batch', 'Serum Batches', 'Stimulant Batches', 'Order ID', 'Priority', 'Code', 'Description', 'Packaging Code', 'Customer', 'Brand',
            ...
            var headerRow = ['Production', '', '', 'Printing', '', '', 'Labelling', '', '', 'Packaging', '', '', 'Shipping', '', ''];
            var secondheader = ['Batch', 'productcode', 'Location', 
                                'Batch', 'productcode', 'Location', 
                                'Batch', 'productcode', 'Location',
                                'Batch', 'productcode', 'Location', 
                                'Batch', 'productcode', 'Location'];
            ...
            for (var i = 0; i < max; i++) {
                if (Production[i]) {
                    ...
                    values.push([Production[i].batch, Production[i].productcode, Production[i].Location]);
            ...
            for (var i = 0; i < max; i++) {
                if (Printing[i]) {
                    ...
                    values.push([Printing[i].batch, Printing[i].productcode, Printing[i].Location]);
            ...
            for (var i = 0; i < max; i++) {
                if (Labelling[i]) {
                    ...
                    values.push([Labelling[i].batch, Labelling[i].productcode, Labelling[i].Location]);
            ...
            for (var i = 0; i < max; i++) {
                if (Packaging[i]) {
                    ...
                    values.push([Packaging[i].batch, Packaging[i].productcode, Packaging[i].Location]);
            ...
            for (var i = 0; i < max; i++) {
                if (shipping[i]) {
                    ...
                    values.push([shipping[i].batch, shipping[i].productcode, shipping[i].Location]);
            ...
            var retArr = [];
            for (var i = 0; i < max; i++) {
                retArr.push(values[i].concat(values[i + max]
                                             .concat(values[i + max + max]
                                                                    .concat(values[i + max + max + max]
                                                                                                 .concat(values[i + max + max + max + max])))));
            ...
            var headerRow = ['SKU', 'Description', 'Order Date', 'Delivery Date', 'Paid Date', 'ETA / Days', 'Order Quantity', 'Note', 'Serum Batch Code', 'Stimulant Batch Code'];
            var keys = ['sku', 'desc', 'orderdate', 'delivdate', 'paiddate', 'eta', 'quantity', 'note', 'serumBatchCode', 'stimulantBatchCode' ]
            var values = [];
            for (var i = 0; i < data.length; i++) {
               var row  = [];
              for(var j = 0 ; j < keys.length; j++){
                if(keys[j] == 'orderdate' || keys[j] == 'delivdate'|| keys[j] == 'paiddate'){
                  row.push( data[i][keys[j]] ? formatDateDisplay(data[i][keys[j]]) : ' ')
                }else{
...
function createCompletedExport(H, name) {
    ...
    var headers = [
        ['Order Date', 'Started', 'Production Machine', 'Labelling Machine', 'Batch', 'OrderID', 'Product Code', 'Product Description', 'Priority', 'Customer', 'Brand', 'Recipe', 'Flavour', 'Bottles',
            'Stocking Amount (litre)', 'Bottle Type', 'Lid Type', 'Packaging Tube',
              'Flavour Value', 'VG Value', 'AG Value', 'PG Value', 'Nicotine Value', 'Nicotine Salts Value', 'CBD Value', 'MCT Value', 'Backed by Mixing',
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            switch (sheets[s]) {
                case 'MixingTeam':
                    ...
                    var customers = [];
                    var orderIDs = [];
                    var result = data[i].Batches ? Object.keys(data[i].Batches).map(function(key) {
...
function exportRows(keys, data, machinesP) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < keys.length; j++) {
            ...
            } else if (keys[j] == 'CompletionDate' || keys[j] == 'runtime' || keys[j] == 'starttime') {
